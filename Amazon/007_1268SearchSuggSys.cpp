class Solution {
public:
    vector<vector<string>> suggestedProducts(vector<string>& pro, string wrd) {
        vector<vector<string>> res;
        sort(pro.begin(),pro.end());
        int start = 0,explored=0,n=pro.size();
        string prefix;
        
        for(auto x: wrd)
        {
            prefix += x;
            start = lower_bound(pro.begin()+explored,pro.end(),prefix) - pro.begin();
            explored = start;
            res.push_back({});
            
            for(int i=start;i<min(start+3,n) && !pro[i].compare(0,prefix.size(),prefix);i++)
                res.back().push_back(pro[i]);
        }
        return res;
    }
};
